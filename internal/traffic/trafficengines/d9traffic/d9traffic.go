...
// getActiveRunway returns the active runway for the given flight context, if not set/available, fallsback to any runway.
// Returns nil if the fallback fails.
func (e *D9TrafficEngine) getActiveRunway(icao string, arrOrDep int) *atc.Runway {
...
func (e *D9TrafficEngine) assignPhaseInitialAltitude(ac *atc.Aircraft, phase int) {
	...
	if rwy == nil {
		rwy = e.getActiveRunway(icao, flightContext)
	}
	ac.Flight.AssignedRunway = rwy
	ac.Flight.AssignedRunwayName = rwy.Name
	...
	case flightphase.Climbout:
		phaseInitAlt = atc.GetElevationAdjustedAltitude(constants.DefaultTakeOffExitClimboutEntryAltFt, ap, rwy, 100.0)
	...
	case flightphase.Departure:
		if sid := ac.Flight.AssignedSID; sid != nil && sid.Entry.ConstraintAlt > 0 {
			...
			phaseInitAlt = atc.GetElevationAdjustedAltitude(constants.DefaultClimbExitDepartureEntryAltFt, ap, rwy, 500.0)
		}
	...
	case flightphase.Arrival:
		...
		if ac.Flight.Position.Altitude > 0 && ac.Flight.Phase.InitialAltitude == 0.0 {
			// if we spawned mid-air inside the Arrival phase, the theoretical starting
			// altitude of this phase is the cruise altitude ceiling
	...
	case flightphase.Approach:
		...
		if ac.Flight.Position.Altitude > 0 && ac.Flight.Phase.InitialAltitude == 0.0 {
			...
			if star := ac.Flight.AssignedSTAR; star != nil && star.Exit.ConstraintAlt > 0 {
				...
				phaseInitAlt = atc.GetElevationAdjustedAltitude(constants.DefaultArrivalExitApproachEntryAltFt, ap, rwy, 500.0)
			}
	...
	case flightphase.Final:
		if rwy != nil && rwy.FAFalt > 0 {
			...
			phaseInitAlt = atc.GetElevationAdjustedAltitude(constants.DefaultApproachExitFinalEntryAltFt, ap, rwy, 500.0)
...
func (e *D9TrafficEngine) updateActiveAircraft(relevantICAOs []string) {
	...
	for _, ac := range e.ActiveAircraft {
		...
		case flightphase.Startup:
			...
			if currSimZTime.After(ac.Flight.Phase.EstimatedNextTransition) {
				ac.Flight.AssignedRunway = e.AirportConfig[airport.ICAO].Departure
		...
		case flightphase.TaxiOut:
			...
			if ac.Flight.Phase.PositionComplete {
				if !e.getRunwayLock(airport, ac.Flight.AssignedRunway, ac) {
		...
		case flightphase.Takeoff:
			...
			if ac.Flight.Phase.PositionComplete {
				e.releaseRunwayLock(airport, ac.Flight.AssignedRunway, ac)
		...
		case flightphase.Cruise:
			...
			// If the aircraft breaches the TOD window or the target boundary,
...
func (e *D9TrafficEngine) updateLinearPosition(ac *atc.Aircraft, ctxAp *atc.Airport) {
	...
	case flightphase.Takeoff:
		...
		targetAlt = atc.GetElevationAdjustedAltitude(constants.DefaultTakeOffExitClimboutEntryAltFt, ctxAp, rwy, 100.0)
	...
	case flightphase.Arrival:
		targetAlt = atc.GetElevationAdjustedAltitude(constants.DefaultArrivalExitApproachEntryAltFt, ctxAp, rwy, 500.0)
		...
		targetHeading = geometry.CalculateBearing(startPos.Lat, startPos.Long, targetPos.Lat, targetPos.Long)
	...
	if phase == flightphase.Approach {
		...
		applySmoothTurnHeading(ac, targetHeading, 3.0, deltaTimeSec)
	...
	if currentDistToTarget <= distanceMovedThisTick {
		...
		bearingDifferenceToTargetPos := math.Abs(geometry.NormalizeHeading(currentBearingToTarget) - ac.Flight.Position.Heading)
		headingBearingDifference := math.Abs(geometry.NormalizeHeading(ac.Flight.TargetHeading) - ac.Flight.Position.Heading)
...
func (e *D9TrafficEngine) updateHoldingPosition(ac *atc.Aircraft, rwy *atc.Runway) {
	...
	for i, a := range stack {
		a.Flight.Holding.TargetHoldAlt = float64(hold.MinAlt + (i * 1000))
		a.Flight.TargetAltitude = a.Flight.Holding.TargetHoldAlt
	}
...
func (e *D9TrafficEngine) updateGoAroundPosition(ac *atc.Aircraft, airport *atc.Airport) {
	...
	safeTurnAlt := (math.Ceil(airport.Elevation+3000.0) / 500.0) * 500.0
...
func (e *D9TrafficEngine) updateCruisePosition(ac *atc.Aircraft) {
	...
	if distToTarget <= requiredDescentDist && altitudeToLose > 0 {
		...
		ac.Flight.TargetAltitude = cruiseAlt
	}
...
func (e *D9TrafficEngine) refreshRunwayConfig(ap *atc.Airport) {
	...
	orientations := e.groupRunwaysByOrientation(viable)
